...
public class AssStatusTypeRef implements ORAData, ORADataFactory {
    ...
    public static AssStatusTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (AssStatusTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to AssStatusTypeRef: " + exn.toString());
        }
    }

    public AssStatusType getValue() throws SQLException {
        return (AssStatusType) AssStatusType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
