...
public class HrDetailTypeRef implements ORAData, ORADataFactory {
    ...
    public static HrDetailTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (HrDetailTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to HrDetailTypeRef: " + exn.toString());
        }
    }

    public HrDetailType getValue() throws SQLException {
        return (HrDetailType) HrDetailType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
