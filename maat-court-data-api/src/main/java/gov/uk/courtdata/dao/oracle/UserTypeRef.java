...
public class UserTypeRef implements ORAData, ORADataFactory {
    ...
    public static UserTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (UserTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to UserTypeRef: " + exn.toString());
        }
    }

    public UserType getValue() throws SQLException {
        return (UserType) UserType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
