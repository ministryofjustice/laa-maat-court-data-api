...
public interface ContributionsRepository extends JpaRepository<ContributionsEntity, Integer> {
    ...
    @Query(
            value =
                    "UPDATE TOGDATA.CONTRIBUTIONS SET REPLACED_DATE = TRUNC(SYSDATE), ACTIVE = 'N' WHERE REP_ID = :repId AND EFFECTIVE_DATE >= :effDate",
            nativeQuery = true)
    ...
    @Query(
            value = "SELECT C.ID, C.MONTHLY_CONTRIBS monthlyContributions, C.UPFRONT_CONTRIBS upfrontContributions, "
                    ...
                    + "WHERE C.REP_ID = :repId ORDER BY C.EFFECTIVE_DATE DESC, C.ID DESC",
            nativeQuery = true)
