...
public class AreaTransferTypeRef implements ORAData, ORADataFactory {
    ...
    public static AreaTransferTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (AreaTransferTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to AreaTransferTypeRef: " + exn.toString());
        }
    }

    public AreaTransferType getValue() throws SQLException {
        return (AreaTransferType) AreaTransferType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
