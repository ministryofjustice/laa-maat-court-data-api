...
public class UserQueryTypeRef implements ORAData, ORADataFactory {
    ...
    public static UserQueryTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (UserQueryTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to UserQueryTypeRef: " + exn.toString());
        }
    }

    public UserQueryType getValue() throws SQLException {
        return (UserQueryType) UserQueryType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
