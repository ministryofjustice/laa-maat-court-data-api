...
public class LoggingMemoryAppender extends ListAppender<ILoggingEvent> {
  ...
  public void assertContains(Matcher<String> stringMatcher, Level level) {
    Optional<ILoggingEvent> matchingLoggingEvent = this.list.stream()
        .filter(loggingEvent -> loggingEvent.getLevel().equals(level))
        .filter(event -> stringMatcher.matches(event.getFormattedMessage()))
        .findFirst();

    assertTrue(matchingLoggingEvent.isPresent(), createFailureMessage(stringMatcher, level));
  }

  private String createFailureMessage(Matcher<String> stringMatcher, Level level) {
