...
public class ThirdPartyTypeRef implements ORAData, ORADataFactory {
    ...
    public static ThirdPartyTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (ThirdPartyTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to ThirdPartyTypeRef: " + exn.toString());
        }
    }

    public ThirdPartyType getValue() throws SQLException {
        return (ThirdPartyType) ThirdPartyType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
