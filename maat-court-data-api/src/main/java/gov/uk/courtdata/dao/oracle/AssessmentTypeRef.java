...
public class AssessmentTypeRef implements ORAData, ORADataFactory {
    ...
    public static AssessmentTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (AssessmentTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to AssessmentTypeRef: " + exn.toString());
        }
    }

    public AssessmentType getValue() throws SQLException {
        return (AssessmentType) AssessmentType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
