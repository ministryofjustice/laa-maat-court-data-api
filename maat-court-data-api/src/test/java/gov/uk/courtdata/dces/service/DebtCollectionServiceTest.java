...
class DebtCollectionServiceTest {
    ...
    void setUp() {
        ...
        contributionFilesEntity =
                TestEntityDataBuilder.getPopulatedContributionFilesEntity(null, "CONTRIBUTIONS_TEST_DATA", xmlDoc);
    }

    @Test
    void givenDateRange_whenGetContributionFiles_thenReturnXmlContents() {
        when(contributionFilesRepository.getByFileNameLikeAndDateCreatedBetweenOrderByFileId(
                        eq("CONTRIBUTIONS%"), any(LocalDate.class), any(LocalDate.class)))
        ...
        assertThat(xmlContents).isNotNull().isNotEmpty();
    }

    @Test
    void givenDateRange_whenGetFdcFiles_thenReturnXmlContents() {
        when(contributionFilesRepository.getByFileNameLikeAndDateCreatedBetweenOrderByFileId(
                        eq("FDC%"), any(LocalDate.class), any(LocalDate.class)))
                .thenReturn(List.of(contributionFilesEntity));
    ...
    void givenFileData_whenSaveError_thenSaveErrorEntity() {
        ...
        ConcorContributionsEntity concorEntity = ConcorContributionsEntity.builder()
                .id(concorId)
    ...
    void givenValidFileId_whenUpdateContributionFileReceivedCount_thenReturnTrueAndIncrementRecordsReceived() {
        ...
        when(contributionFilesRepository.incrementRecordsReceived(fileId, "DCES"))
                .thenReturn(1);
        assertThat(debtCollectionService.updateContributionFileReceivedCount(fileId))
                .isTrue();
    ...
    void givenInvalidFileId_whenUpdateContributionFileReceivedCount_thenThrowException() {
        ...
        when(contributionFilesRepository.incrementRecordsReceived(fileId, "DCES"))
                .thenReturn(0);
        assertThatThrownBy(() -> debtCollectionService.updateContributionFileReceivedCount(fileId))
