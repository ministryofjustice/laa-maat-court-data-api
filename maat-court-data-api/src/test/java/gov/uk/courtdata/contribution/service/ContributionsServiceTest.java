...
class ContributionsServiceTest {
    ...
    void givenAValidRepId_whenFindIsInvoked_thenContributionsEntryIsRetrieved() {
        when(repository.findByRepOrder_IdAndLatestIsTrue(anyInt()))
                .thenReturn(ContributionsEntity.builder()
    ...
    void givenAValidRepIdAndFindAllRepId_whenFindIsInvoked_thenContributionsEntryIsRetrieved() {
        when(repository.findAllByRepOrder_Id(anyInt()))
                .thenReturn(List.of(ContributionsEntity.builder()
                        .repOrder(TestEntityDataBuilder.getPopulatedRepOrder(REP_ID))
                        .build()));
    ...
    void givenAValidContribution_whenCreateIsInvoked_thenContributionsEntryIsCreated() {
        ...
        when(contributionsMapper.createContributionsToContributionsEntity(any(CreateContributionRequest.class)))
    ...
    @Test
    void
            givenContributionsEntryAlreadyExists_whenCreateIsInvoked_thenExistingEntryIsUpdated_andNewContributionsEntryIsCreated() {
        LocalDate testEffectiveDate = LocalDate.now();
        ContributionsEntity contributionsEntity = ContributionsEntity.builder().build();
        when(repository.findByRepOrder_IdAndLatestIsTrue(TestModelDataBuilder.REP_ID))
                .thenReturn(contributionsEntity);
        when(contributionsMapper.createContributionsToContributionsEntity(any(CreateContributionRequest.class)))
                .thenReturn(contributionsEntity);
        when(repository.saveAndFlush(any(ContributionsEntity.class))).thenReturn(contributionsEntity);
        when(contributionsMapper.mapEntityToDTO(any(ContributionsEntity.class)))
                .thenReturn(ContributionsDTO.builder()
                        .repId(TestModelDataBuilder.REP_ID)
                        .build());

        contributionsService.create(new CreateContributionRequest()
                .withRepId(TestModelDataBuilder.REP_ID)
                .withEffectiveDate(testEffectiveDate));
    ...
    void givenAValidRepId_whenGetContributionsSummaryIsInvoked_thenContributionsSummaryIsReturned() {
        List<ContributionsSummaryView> contributionsSummaryViewEntities =
                List.of(TestModelDataBuilder.getContributionsSummaryView());
        when(repository.getContributionsSummary(TestModelDataBuilder.REP_ID))
                .thenReturn(contributionsSummaryViewEntities);
