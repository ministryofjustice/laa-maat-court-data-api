...
public class DefendantValidator implements IValidator<DefendantMAATDataEntity, Integer> {
    ...
    public Optional<DefendantMAATDataEntity> validate(Integer maatId) {
        ...
        final Optional<DefendantMAATDataEntity> defendantViewEntity = defendantMAATDataRepository.findBymaatId(maatId);

        if (defendantViewEntity.isEmpty()) throw new ValidationException("MAAT Defendant details not found.");
