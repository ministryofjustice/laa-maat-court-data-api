...
public class FrequencyTypeRef implements ORAData, ORADataFactory {
    ...
    public static FrequencyTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (FrequencyTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to FrequencyTypeRef: " + exn.toString());
        }
    }

    public FrequencyType getValue() throws SQLException {
        return (FrequencyType) FrequencyType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
