...
public class ReservationTypeRef implements ORAData, ORADataFactory {
    ...
    public static ReservationTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (ReservationTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to ReservationTypeRef: " + exn.toString());
        }
    }

    public ReservationType getValue() throws SQLException {
        return (ReservationType) ReservationType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
