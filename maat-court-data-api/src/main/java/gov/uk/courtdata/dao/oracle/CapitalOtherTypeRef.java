...
public class CapitalOtherTypeRef implements ORAData, ORADataFactory {
    ...
    public static CapitalOtherTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (CapitalOtherTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to CapitalOtherTypeRef: " + exn.toString());
        }
    }

    public CapitalOtherType getValue() throws SQLException {
        return (CapitalOtherType) CapitalOtherType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
