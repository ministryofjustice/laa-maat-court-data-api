...
public class EquityTypeRef implements ORAData, ORADataFactory {
    ...
    public static EquityTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (EquityTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to EquityTypeRef: " + exn.toString());
        }
    }

    public EquityType getValue() throws SQLException {
        return (EquityType) EquityType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
