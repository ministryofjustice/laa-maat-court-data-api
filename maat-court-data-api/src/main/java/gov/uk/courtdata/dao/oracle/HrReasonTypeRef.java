...
public class HrReasonTypeRef implements ORAData, ORADataFactory {
    ...
    public static HrReasonTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (HrReasonTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to HrReasonTypeRef: " + exn.toString());
        }
    }

    public HrReasonType getValue() throws SQLException {
        return (HrReasonType) HrReasonType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
