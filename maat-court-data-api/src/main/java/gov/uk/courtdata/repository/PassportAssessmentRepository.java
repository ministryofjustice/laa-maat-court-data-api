...
public interface PassportAssessmentRepository extends JpaRepository<PassportAssessmentEntity, Integer> {
    ...
    @Query(
            value =
                    "UPDATE TOGDATA.PASSPORT_ASSESSMENTS pa set pa.REPLACED = 'Y' WHERE pa.ID <> :id AND pa.REP_ID = :repId",
            nativeQuery = true)
    void replaceAllByRepIdExcludingPassportedAssessment(@Param("repId") Integer repId, @Param("id") Integer id);
