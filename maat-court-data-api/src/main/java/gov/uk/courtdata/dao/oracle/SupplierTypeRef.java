...
public class SupplierTypeRef implements ORAData, ORADataFactory {
    ...
    public static SupplierTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (SupplierTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to SupplierTypeRef: " + exn.toString());
        }
    }

    public SupplierType getValue() throws SQLException {
        return (SupplierType) SupplierType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
