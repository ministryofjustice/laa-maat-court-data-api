...
public class ConcorContributionsService {
    ...
    public List<Integer> updateConcorContributionStatusAndResetContribFile(
        ...
        List<Integer> idsToUpdate =
                concorRepository
                        .findByStatusAndFullXmlIsNotNullOrderByIdDesc(SENT, Pageable.ofSize(request.getRecordCount()))
                        .stream()
                        .map(ConcorContributionsRepository.IdOnly::getId)
                        .toList();
    ...
    public List<ConcorContributionResponse> getConcorContributionFiles(
            ConcorContributionStatus status, Integer noOfRecords, Integer concorContributionId) {

        Integer finalConcorContributionId =
                Optional.ofNullable(concorContributionId).orElse(0);
        ...
        log.info(
                "Searching concor contribution file with status {}, startId {} and count {}",
                status,
                concorContributionId,
                noOfRecords);
        Pageable pageable = PageRequest.of(0, noOfRecords, Sort.by("id"));
        List<Integer> idList =
                concorRepository.findByStatusAndIdGreaterThan(status, finalConcorContributionId, pageable).stream()
                        .map(ConcorContributionsRepository.IdOnly::getId)
                        .toList();
    ...
    private static List<ConcorContributionResponse> buildConcorContributionResponseList(
        ...
        return repositoryCall.get().stream()
                .map(cc -> ConcorContributionResponse.builder()
    ...
    public ConcorContributionResponse getConcorContributionFile(Integer concorContributionId) {
        ...
        return concorFile
                ...
                .orElseThrow(() -> new RequestedObjectNotFoundException(
                        "Concor Contribution ID " + concorContributionId + " not found"));
    }
    ...
    public Integer createContributionAndUpdateConcorStatus(CreateContributionFileRequest contributionRequest) {
        ...
        ValidationUtils.isEmptyOrHasNullElement(
                contributionRequest.getConcorContributionIds(), "ContributionIds are empty/null.");
        final ContributionFilesEntity contributionFilesEntity = createContributionFile(contributionRequest);
        if (!updateConcorContributionStatusAndSetContribFile(
                contributionRequest.getConcorContributionIds(), SENT, contributionFilesEntity.getFileId())) {
            throw new NoSuchElementException(
                    "No concor_contribution status values were updated"); // did not rollback previously
        }
        if (contributionFilesEntity.getFileId() == null) {
            throw new RequestedObjectNotFoundException(
                    "Created contribution_file's id could not be found"); // did not rollback previously
        }
    ...
    public Integer logContributionProcessed(LogContributionProcessedRequest request) {
        ConcorContributionsEntity concorEntity = concorRepository
                .findById(request.getConcorId())
                .orElseThrow(() -> new RequestedObjectNotFoundException(
                        "log concor_contribution ID " + request.getConcorId() + ": not found"));
        ...
        if (!StringUtils.isEmpty(request.getErrorText())) {
            if (concorEntity.getContribFileId() == null) {
                // returns HTTP status 400 with error code "Object Not Found" (rather than "DB error" as it would
                // without this check).
    ...
    public ConcorContributionResponseDTO getConcorContribution(Integer concorContributionId) {
        ...
        if (concorContributionEntity.isPresent()) {
            log.info(
                    "Concor Contribution found: {}",
                    concorContributionEntity.get().getId());
    ...
    private ContributionFilesEntity createContributionFile(CreateContributionFileRequest contributionRequest) {
        ...
        try {
            ...
            throw new MAATCourtDataException(
                    "Failed to map ConcorContributionRequest to ContributionFilesEntity and persist: [%s]"
    ...
    private boolean updateConcorContributionStatusAndSetContribFile(
        ...
        if (!concorFileList.isEmpty()) {
            ...
            log.info(
                    "Updated all concor contribution file for contributionFileId {} and count {}",
                    contributionFileId,
                    concorFileList.size());
            return true;
        } else {
