...
public class EvidenceFeeTypeRef implements ORAData, ORADataFactory {
    ...
    public static EvidenceFeeTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (EvidenceFeeTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to EvidenceFeeTypeRef: " + exn.toString());
        }
    }

    public EvidenceFeeType getValue() throws SQLException {
        return (EvidenceFeeType) EvidenceFeeType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
