...
class RepOrderMvoImplTest {
    ...
    @Test
    public void
            givenRepIdAndVehicleOwner_whenFindRepOrderMvoByRepIdAndVehicleOwnerIsInvoked_thenRepOrderMvoEntityInfoIsRetrieved() {
        repOrderMvoImpl.findRepOrderMvoByRepIdAndVehicleOwner(TestModelDataBuilder.REP_ID, VEHICLE_OWNER_INDICATOR_YES);
        verify(repOrderMvoRepository, times(1))
                .findByRepIdAndVehicleOwner(
                        TestModelDataBuilder.REP_ID, VEHICLE_OWNER_INDICATOR_YES, RepOrderMvoEntityInfo.class);
    }
}
