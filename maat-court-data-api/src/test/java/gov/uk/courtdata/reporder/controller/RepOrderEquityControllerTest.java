...
public class RepOrderEquityControllerTest {
    ...
    private static final RepOrderEquityEntity REP_ORDER_EQUITY =
    ...
    void givenID_whenGetRepOrderEquityCalled_thenReturnRepOrderEquity() throws Exception {
        when(mockRepOrderEquityService.retrieve(ID)).thenReturn(REP_ORDER_EQUITY);

        mvc.perform(MockMvcRequestBuilders.get(BASE_ENDPOINT_FORMAT + "/" + ID).contentType(MediaType.APPLICATION_JSON))
    ...
    void givenNonExistentID_whenGetRepOrderEquityCalled_thenReturnAnError() throws Exception {
        when(mockRepOrderEquityService.retrieve(NON_EXISTENT_ID))
                .thenThrow(
                        new RequestedObjectNotFoundException("No Rep Order Equity found with ID: " + NON_EXISTENT_ID));

        mvc.perform(MockMvcRequestBuilders.get(BASE_ENDPOINT_FORMAT + "/" + NON_EXISTENT_ID)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(content()
                        .json("{\"code\":\"NOT_FOUND\",\"message\":\"No Rep Order Equity found with ID: "
                                + NON_EXISTENT_ID + "\"}"));
    }
