...
public class Di_audit_typeRef implements ORAData, ORADataFactory {
    ...
    public static Di_audit_typeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (Di_audit_typeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to Di_audit_typeRef: " + exn.toString());
        }
    }

    public Di_audit_type getValue() throws SQLException {
        return (Di_audit_type) Di_audit_type.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
