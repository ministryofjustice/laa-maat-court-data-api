...
import gov.uk.courtdata.dao.oracle.ThirdPartyTabtype;
import gov.uk.courtdata.dao.oracle.ThirdPartyType;
...
public class PropertyConvertor extends Convertor {
     ...
     */
    @Override
    public PropertyType getOracleType() throws MAATApplicationException, MAATSystemException {
        if (getDbType() == null) {
    ...
    public void setDTO(Object dto) throws MAATApplicationException {
        if (dto instanceof PropertyDTO) this.setDto(dto);
        else
            throw new MAATApplicationException(" Invalid DTO type for conversion got "
                    + dto.getClass().getName() + " wanted " + PropertyDTO.class.getName());
    }
    ...
    public void setDTOFromType(Object oracleType) throws MAATApplicationException, MAATSystemException {
        ...
        try {
            ...
            getDTO().setPercentageOwnedApplicant(
                            convertorHelper.toDouble(getOracleType().getPercentageOwnedApplicant()));
            getDTO().setPercentageOwnedPartner(
                            convertorHelper.toDouble(getOracleType().getPercentageOwnedPartner()));
            getDTO().setDeclaredMortgageCharges(
                            convertorHelper.toCurrency(getOracleType().getDeclaredMortgageCharges()));
            getDTO().setDeclaredMarketValue(
                            convertorHelper.toCurrency(getOracleType().getDeclaredMarketValue()));
            getDTO().setVerifiedMortgageCharges(
                            convertorHelper.toCurrency(getOracleType().getVerifiedMortgageCharges()));
            getDTO().setVerifiedMarketValue(
                            convertorHelper.toCurrency(getOracleType().getVerifiedMarketValue()));
            getDTO().setVerifiedEquityAmount(
                            convertorHelper.toCurrency(getOracleType().getVerifiedEquityAmount()));
            getDTO().setDeclaredEquityAmount(
                            convertorHelper.toCurrency(getOracleType().getDeclaredEquityAmount()));
    ...
    public void setTypeFromDTO(Object dto) throws MAATApplicationException, MAATSystemException {
        ...
        try {
            ...
            getOracleType()
                    .setPercentageOwnedApplicant(convertorHelper.toDouble(getDTO().getPercentageOwnedApplicant()));
            getOracleType().setPercentageOwnedPartner(convertorHelper.toDouble(getDTO().getPercentageOwnedPartner()));
            getOracleType()
                    .setDeclaredMortgageCharges(convertorHelper.toCurrency(getDTO().getDeclaredMortgageCharges()));
            getOracleType().setDeclaredMarketValue(convertorHelper.toCurrency(getDTO().getDeclaredMarketValue()));
            getOracleType()
                    .setVerifiedMortgageCharges(convertorHelper.toCurrency(getDTO().getVerifiedMortgageCharges()));
            ...
            if ((getDTO().getThirdPartyOwners() != null) && (getDTO().getThirdPartyOwners()).size() > 0) {
                ...
                ThirdPartyType[] thirdPartyTypes =
                        new ThirdPartyType[getDTO().getThirdPartyOwners().size()];
                Iterator<ThirdPartyOwnerDTO> thirdParties =
                        getDTO().getThirdPartyOwners().iterator();
