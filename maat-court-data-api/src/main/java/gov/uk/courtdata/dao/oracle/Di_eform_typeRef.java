...
public class Di_eform_typeRef implements ORAData, ORADataFactory {
    ...
    public static Di_eform_typeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (Di_eform_typeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to Di_eform_typeRef: " + exn.toString());
        }
    }

    public Di_eform_type getValue() throws SQLException {
        return (Di_eform_type) Di_eform_type.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
