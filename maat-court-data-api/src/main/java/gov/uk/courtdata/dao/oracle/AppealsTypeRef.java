...
public class AppealsTypeRef implements ORAData, ORADataFactory {
    ...
    public static AppealsTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (AppealsTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to AppealsTypeRef: " + exn.toString());
        }
    }

    public AppealsType getValue() throws SQLException {
        return (AppealsType) AppealsType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
