...
public class AreaTypeRef implements ORAData, ORADataFactory {
    ...
    public static AreaTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (AreaTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to AreaTypeRef: " + exn.toString());
        }
    }

    public AreaType getValue() throws SQLException {
        return (AreaType) AreaType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
