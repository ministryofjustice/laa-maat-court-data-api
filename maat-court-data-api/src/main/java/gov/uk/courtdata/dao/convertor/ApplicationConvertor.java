...
public class ApplicationConvertor extends Convertor {
     ...
     */
    @Override
    public ApplicationType getOracleType() throws MAATApplicationException, MAATSystemException {
        if (getDbType() == null) {
    ...
    public void setDTO(Object dto) throws MAATApplicationException {
        if (dto instanceof ApplicationDTO) this.setDto(dto);
        else
            throw new MAATApplicationException(" Invalid DTO type for conversion got "
                    + dto.getClass().getName() + " wanted " + ApplicationDTO.class.getName());
    }
    ...
    public void setDTOFromType(Object oracleType) throws MAATApplicationException, MAATSystemException {
        ...
        try {
            ...
            getDTO().setSolicitorAdminEmail(
                            convertorHelper.toString(getOracleType().getSolicitorAdminEmail()));

            getDTO().setAlertMessage(convertorHelper.toString(getOracleType().getAlertMessage()));
            getDTO().setDateStatusSet(
                            convertorHelper.toSysGenDate(getOracleType().getDateStatusSet()));
            ...
            getDTO().setMagsCourtOutcomeDate(
                            convertorHelper.toSysGenDate(getOracleType().getMagsCourtOutcomeDate()));
            getDTO().setMagsWithdrawalDate(
                            convertorHelper.toDate(getOracleType().getMagsWithdrawalDate()));
            ...
            getDTO().setWelshCorrepondence(
                            convertorHelper.toBoolean(getOracleType().getWelshCorrespondence()));
            getDTO().setDecisionDate(convertorHelper.toDate(getOracleType().getDecisionDate()));
            getDTO().setApplicantHasPartner(
                            convertorHelper.toBoolean(getOracleType().getPartner()));
            ...
            getDTO().setPartnerContraryInterestDTO(
                            ciConvertor.getDTO()); // if type is null this will initialise a valid empty object
    ...
    public void setTypeFromDTO(Object dto) throws MAATApplicationException, MAATSystemException {
        ...
        try {
            ...
            getOracleType()
                    .setContraryInterestObject(
                            ciConvertor.getOracleType()); // if type is null this will initialise a valid empty object
            ...
            AssessmentSummaryConvertor convertor = new AssessmentSummaryConvertor();
            AssessmentSummaryType[] assessmentSummaryTypes =
                    new AssessmentSummaryType[getDTO().getAssessmentSummary().size()];
            if ((getDTO().getAssessmentSummary() != null)
                    && (getDTO().getAssessmentSummary().size() > 0)) {
                Iterator<AssessmentSummaryDTO> asit =
                        getDTO().getAssessmentSummary().iterator();
                int idx = 0;
                while (asit.hasNext()) {
            ...
            ApplicantLinkConvertor alconvertor = new ApplicantLinkConvertor();
            ApplicantLinkType[] applicantLinkTypes =
                    new ApplicantLinkType[getDTO().getApplicantLinks().size()];
            if ((getDTO().getApplicantLinks() != null)
                    && (getDTO().getApplicantLinks().size() > 0)) {
            ...
            // MW - 30/03/2017 - FIP Changes
            DigitalMeansAssessmentConvertor dmaConvertor = new DigitalMeansAssessmentConvertor();
            DigiMeansAssessType[] digiMeansAssessmentTypes =
                    new DigiMeansAssessType[getDTO().getMeansAssessments().size()];

            if ((getDTO().getMeansAssessments() != null)
                    && (getDTO().getMeansAssessments().size() > 0)) {
                Iterator<DigitisedMeansAssessmentDTO> dmaIt =
                        getDTO().getMeansAssessments().iterator();
                int idx = 0;
                while (dmaIt.hasNext()) {
