...
import gov.uk.courtdata.dao.oracle.ApplPaymentDetailsType;
import gov.uk.courtdata.dao.oracle.ContributionSummaryTabType;
import gov.uk.courtdata.dao.oracle.ContributionSummaryType;
import gov.uk.courtdata.dao.oracle.ContributionsType;
import gov.uk.courtdata.dao.oracle.CorrespondenceTabType;
import gov.uk.courtdata.dao.oracle.CorrespondenceType;
import gov.uk.courtdata.dao.oracle.CrownCourtOverviewType;
...
public class CrownCourtOverviewConvertor extends Convertor {
     ...
     */
    @Override
    public CrownCourtOverviewType getOracleType() throws MAATApplicationException, MAATSystemException {
        if (getDbType() == null) {
    ...
    public void setDTO(Object dto) throws MAATApplicationException {
        if (dto instanceof CrownCourtOverviewDTO) this.setDto(dto);
        else
            throw new MAATApplicationException(" Invalid DTO type for conversion got "
                    + dto.getClass().getName() + " wanted " + CrownCourtOverviewDTO.class.getName());
    }
    ...
    public void setTypeFromDTO(Object dto) throws MAATApplicationException, MAATSystemException {
        ...
        try {
            ...
            if ((getDTO().getCorrespondence() != null)
                    && (getDTO().getCorrespondence().size() > 0)) {
                CorrespondenceType[] ct =
                        new CorrespondenceType[getDTO().getCorrespondence().size()];
            ...
            if ((getDTO().getContributionSummary() != null)
                    && (getDTO().getContributionSummary().size() > 0)) {
                ContributionSummaryType[] ct = new ContributionSummaryType
                        [getDTO().getContributionSummary().size()];
                Iterator<ContributionSummaryDTO> cIt =
                        getDTO().getContributionSummary().iterator();
                int idx = 0;
