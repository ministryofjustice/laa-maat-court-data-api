...
public class OffenceTypeTypeRef implements ORAData, ORADataFactory {
    ...
    public static OffenceTypeTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (OffenceTypeTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to OffenceTypeTypeRef: " + exn.toString());
        }
    }

    public OffenceTypeType getValue() throws SQLException {
        return (OffenceTypeType) OffenceTypeType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
