...
public class PrintDatesTypeRef implements ORAData, ORADataFactory {
    ...
    public static PrintDatesTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (PrintDatesTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to PrintDatesTypeRef: " + exn.toString());
        }
    }

    public PrintDatesType getValue() throws SQLException {
        return (PrintDatesType) PrintDatesType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
