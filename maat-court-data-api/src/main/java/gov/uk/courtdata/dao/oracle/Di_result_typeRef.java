...
public class Di_result_typeRef implements ORAData, ORADataFactory {
    ...
    public static Di_result_typeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (Di_result_typeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to Di_result_typeRef: " + exn.toString());
        }
    }

    public Di_result_type getValue() throws SQLException {
        return (Di_result_type) Di_result_type.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
