...
public class PropertyTypeRef implements ORAData, ORADataFactory {
    ...
    public static PropertyTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (PropertyTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to PropertyTypeRef: " + exn.toString());
        }
    }

    public PropertyType getValue() throws SQLException {
        return (PropertyType) PropertyType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
