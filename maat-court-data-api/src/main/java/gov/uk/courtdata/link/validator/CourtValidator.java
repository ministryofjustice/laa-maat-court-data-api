...
public class CourtValidator implements IValidator<Void, CaseDetails> {
    ...
    public Optional<Void> validate(final CaseDetails caseDetailsJson) {
        ...
        if (isBlank(caseDetailsJson.getCjsAreaCode())) throw new ValidationException("cjs area code not found.");

        List<Session> sessions = caseDetailsJson.getSessions();

        if (sessions == null || sessions.isEmpty()) throw new ValidationException("Sessions not available.");

        sessions.forEach(s -> Optional.ofNullable(s.getCourtLocation())
