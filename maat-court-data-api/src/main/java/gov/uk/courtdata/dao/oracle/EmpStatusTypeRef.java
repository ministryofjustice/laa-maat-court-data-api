...
public class EmpStatusTypeRef implements ORAData, ORADataFactory {
    ...
    public static EmpStatusTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (EmpStatusTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to EmpStatusTypeRef: " + exn.toString());
        }
    }

    public EmpStatusType getValue() throws SQLException {
        return (EmpStatusType) EmpStatusType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
