...
public class OffenceHelper {
    ...
    public List<OffenceSummary> getTrialOffences(List<OffenceSummary> offenceList, int maatId) {
        ...
        List<Integer> committalForTrialRefResults =
                xlatResultRepository.findResultsByWQType(WQType.COMMITTAL_QUEUE.value(), COMMITTAL_FOR_TRIAL_SUB_TYPE);
        List<Integer> committalForSentenceRefResults = xlatResultRepository.findResultsByWQType(
    ...
    private boolean isNewCCOffence(
            ...
            List<OffenceEntity> offenceEntities,
            List<Integer> committalForSentenceRefResults,
            int caseId) {
        boolean isNewCCOffence = false;

        int newOffenceCount = offenceRepository.getNewOffenceCount(
                        caseId, offence.getOffenceId().toString())
                + wqOffenceRepository.getNewOffenceCount(
                        caseId, offence.getOffenceId().toString());

        if (newOffenceCount > 0 && !hasCommittalResults(offence, offenceEntities, committalForSentenceRefResults)) {
            isNewCCOffence = true;
        }
    ...
    private boolean hasCommittalResults(
            OffenceSummary offence, List<OffenceEntity> offenceEntities, List<Integer> committalRefResults) {
        OffenceEntity offenceEntity = offenceEntities.stream()
    ...
    private boolean hasResults(OffenceEntity offenceEntity, List<Integer> committalRefResults) {
        ...
        if (offenceEntity != null) {
            ...
            List<Integer> resultList =
                    resultRepository.findResultCodeByCaseIdAndAsnSeq(offenceEntity.getCaseId(), asnSeq);
            List<Integer> wqResultList =
                    wqResultRepository.findResultCodeByCaseIdAndAsnSeq(offenceEntity.getCaseId(), asnSeq);
    ...
    private String getAsnSeq(OffenceEntity offenceEntity) {
        ...
        return asnSeq < 9
                ? offenceEntity.getAsnSeq().substring(2)
                : offenceEntity.getAsnSeq().substring(1);
    }

    public boolean isNewOffence(Integer caseId, String asaSeq) {

        Integer offenceCount = offenceRepository.getOffenceCountForAsnSeq(
                caseId, String.format(LEADING_ZERO_3, Integer.parseInt(asaSeq)));
