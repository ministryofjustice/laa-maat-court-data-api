...
public class ApplicationTypeRef implements ORAData, ORADataFactory {
    ...
    public static ApplicationTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (ApplicationTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to ApplicationTypeRef: " + exn.toString());
        }
    }

    public ApplicationType getValue() throws SQLException {
        return (ApplicationType) ApplicationType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
