...
import gov.uk.courtdata.dao.oracle.CapitalEquityType;
import gov.uk.courtdata.dao.oracle.CapitalOtherTabType;
import gov.uk.courtdata.dao.oracle.CapitalOtherType;
import gov.uk.courtdata.dao.oracle.CapitalPropertiesTabType;
import gov.uk.courtdata.dao.oracle.CapitalPropertyType;
import gov.uk.courtdata.dao.oracle.EquityTabtype;
...
public class CapitalEquityConvertor extends Convertor {
     ...
     */
    @Override
    public CapitalEquityType getOracleType() throws MAATApplicationException, MAATSystemException {
        if (getDbType() == null) {
    ...
    public void setDTO(Object dto) throws MAATApplicationException {
        if (dto instanceof CapitalEquityDTO) this.setDto(dto);
        else
            throw new MAATApplicationException(" Invalid DTO type for conversion got "
                    + dto.getClass().getName() + " wanted " + CapitalEquityDTO.class.getName());
    }
    ...
    public void setDTOFromType(Object oracleType) throws MAATApplicationException, MAATSystemException {
        ...
        try {
            ...
            getDTO().setNoCapitalDeclared(
                            convertorHelper.toBoolean(getOracleType().getNoCapitalDeclared()));
            getDTO().setSuffientVeriToCoverCase(
                            convertorHelper.toBoolean(getOracleType().getSufficientVeriToCoverCase()));
            getDTO().setVerifiedEquityToCoverCase(
                            convertorHelper.toBoolean(getOracleType().getVerifiedEquityToCoverCase()));
            getDTO().setSuffientDeclToCoverCase(
                            convertorHelper.toBoolean(getOracleType().getSufficientDeclToCoverCase()));
            getDTO().setDeclaredEquityToCoverCase(
                            convertorHelper.toBoolean(getOracleType().getDeclaredEquityToCoverCase()));
    ...
    public void setTypeFromDTO(Object dto) throws MAATApplicationException, MAATSystemException {
        ...
        try {
            ...
            getOracleType()
                    .setSufficientVeriToCoverCase(convertorHelper.toBoolean(getDTO().getSuffientVeriToCoverCase()));
            getOracleType()
                    .setVerifiedEquityToCoverCase(convertorHelper.toBoolean(getDTO().getVerifiedEquityToCoverCase()));
            getOracleType()
                    .setSufficientDeclToCoverCase(convertorHelper.toBoolean(getDTO().getSuffientDeclToCoverCase()));
            getOracleType()
                    .setDeclaredEquityToCoverCase(convertorHelper.toBoolean(getDTO().getDeclaredEquityToCoverCase()));
            ...
            if (getDTO().getEquityObjects() != null) {
                EquityType[] equityObjects =
                        new EquityType[getDTO().getEquityObjects().size()];
            ...
            if (getDTO().getCapitalProperties() != null) {
                CapitalPropertyType[] capitalProperties =
                        new CapitalPropertyType[getDTO().getCapitalProperties().size()];

                Iterator<CapitalPropertyDTO> it =
                        getDTO().getCapitalProperties().iterator();
                int idx = 0;
                while ((it != null) && (it.hasNext())) {
                    CapitalPropertyConvertor cpConvertor = new CapitalPropertyConvertor();
            ...
            if (getDTO().getCapitalOther() != null) {
                CapitalOtherType[] capitalOtherTypes =
                        new CapitalOtherType[getDTO().getCapitalOther().size()];
