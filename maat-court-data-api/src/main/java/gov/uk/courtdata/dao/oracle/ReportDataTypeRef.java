...
public class ReportDataTypeRef implements ORAData, ORADataFactory {
    ...
    public static ReportDataTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (ReportDataTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to ReportDataTypeRef: " + exn.toString());
        }
    }

    public ReportDataType getValue() throws SQLException {
        return (ReportDataType) ReportDataType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
