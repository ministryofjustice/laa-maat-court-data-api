...
public class EvidenceTypeTypeRef implements ORAData, ORADataFactory {
    ...
    public static EvidenceTypeTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (EvidenceTypeTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to EvidenceTypeTypeRef: " + exn.toString());
        }
    }

    public EvidenceTypeType getValue() throws SQLException {
        return (EvidenceTypeType) EvidenceTypeType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
