...
public class DisabilityTypeRef implements ORAData, ORADataFactory {
    ...
    public static DisabilityTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (DisabilityTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to DisabilityTypeRef: " + exn.toString());
        }
    }

    public DisabilityType getValue() throws SQLException {
        return (DisabilityType) DisabilityType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
