...
public class UserSessionTypeRef implements ORAData, ORADataFactory {
    ...
    public static UserSessionTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (UserSessionTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to UserSessionTypeRef: " + exn.toString());
        }
    }

    public UserSessionType getValue() throws SQLException {
        return (UserSessionType) UserSessionType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
