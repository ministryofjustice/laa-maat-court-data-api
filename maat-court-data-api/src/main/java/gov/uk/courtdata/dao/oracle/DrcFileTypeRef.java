...
public class DrcFileTypeRef implements ORAData, ORADataFactory {
    ...
    public static DrcFileTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (DrcFileTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to DrcFileTypeRef: " + exn.toString());
        }
    }

    public DrcFileType getValue() throws SQLException {
        return (DrcFileType) DrcFileType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
