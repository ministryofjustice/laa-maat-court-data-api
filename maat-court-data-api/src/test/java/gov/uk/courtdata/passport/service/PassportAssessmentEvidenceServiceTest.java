...
class PassportAssessmentEvidenceServiceTest {
    ...
    void givenValidPassportAssessmentId_whenFindIsInvoked_thenEvidenceIsSeparatedByApplicantAndPartnerAndReturned() {
        ...
        ApiGetPassportEvidenceResponse apiGetPassportEvidenceResponse =
        ...
        when(passportAssessmentPersistenceService.find(LEGACY_PASSPORT_ASSESSMENT_ID))
                .thenReturn(passportAssessmentEntity);
        when(partnerResolver.getPartnerLegacyId(
                        passportAssessmentEntity.getRepOrder().getId()))
                .thenReturn(PARTNER_ID);
        when(passportAssessmentEvidenceMapper.toApiGetPassportEvidenceResponse(passportAssessmentEntity))
                .thenReturn(apiGetPassportEvidenceResponse);
        when(passportAssessmentEvidenceMapper.toApiIncomeEvidence(applicantEvidenceEntity))
                .thenReturn(applicantApiGetIncomeEvidenceResponse);
        when(passportAssessmentEvidenceMapper.toApiIncomeEvidence(partnerEvidenceEntity))
                .thenReturn(partnerApiGetIncomeEvidenceResponse);
    ...
    void givenValidPassportAssessmentIdWithNoPartnerEvidence_whenFindIsInvoked_thenApplicantEvidenceIsReturned() {
        ...
        assertThat(apiGetPassportEvidenceResponse.getPartnerEvidenceItems()).isEmpty();
    }
    ...
    void givenNoApplicantIdOnEvidence_whenFindIsInvoked_thenExceptionIsThrown() {
        ...
        assertThatThrownBy(() -> passportAssessmentEvidenceService.find(LEGACY_PASSPORT_ASSESSMENT_ID))
                .isInstanceOf(InvalidPassportEvidenceStateException.class)
