...
class StoredProcedureControllerTest {
    ...
    @Test
    void givenAValidContentAndErrorWhileExecuteStoredProcedure_whenExecuteStoredProcedureIsInvoked_thenErrorIsThrown()
            throws Exception {
    ...
    void givenAValidInput_whenExecuteStoredProcedureIsInvoked_thenReturnStatusOK() throws Exception {
        when(service.executeStoredProcedure(any()))
                .thenReturn(ApplicationDTO.builder()
                        .repId(TestModelDataBuilder.REP_ID.longValue())
                        .build());
        mvc.perform(MockMvcRequestBuilders.post(ENDPOINT_URL)
                        .content(objectMapper.writeValueAsBytes(
                                StoredProcedureRequest.builder().build()))
