...
public class ReportsTypeRef implements ORAData, ORADataFactory {
    ...
    public static ReportsTypeRef cast(ORAData o) throws SQLException {
        ...
        try {
            return (ReportsTypeRef) getORADataFactory().create(o.toDatum(null), OracleTypes.REF);
        } catch (Exception exn) {
            throw new SQLException(
                    "Unable to convert " + o.getClass().getName() + " to ReportsTypeRef: " + exn.toString());
        }
    }

    public ReportsType getValue() throws SQLException {
        return (ReportsType) ReportsType.getORADataFactory().create(_ref.getSTRUCT(), OracleTypes.REF);
    }
